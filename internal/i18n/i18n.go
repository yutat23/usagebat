...
var messages = map[string][2]string{
	...
	"save":                    {"Save", "保存"},
	"windowsLayout":           {"Windows icon layout", "Windowsアイコンの配置"},
	"layoutStack":             {"Stack multiple limits", "複数の制限を積み重ねる"},
	"layoutSingle":            {"Show the tightest limit", "最も厳しい制限だけ表示"},
	"refreshInterval":         {"Refresh interval", "更新間隔"},
	"secondsDetail":           {"Seconds (5–3600)", "秒（5〜3600）"},
	"notificationSettings":    {"Notifications", "通知"},
	"expiryThresholds":        {"Expiry warning times", "期限切れの通知タイミング"},
	"hoursListDetail":         {"Hours before expiry, separated by commas", "期限切れまでの時間をカンマ区切りで指定"},
	"remainingThresholds":     {"Low-limit thresholds", "残量の通知しきい値"},
	"percentListDetail":       {"Remaining percentages, separated by commas", "残量%をカンマ区切りで指定"},
	"historySettings":         {"History", "履歴"},
	"historyInterval":         {"Recording interval", "記録間隔"},
	"historyRetention":        {"Retention period", "保存期間"},
	"minutesDetail":           {"Minutes", "分"},
	"daysDetail":              {"Days", "日"},
	"updateSettings":          {"Updates", "更新確認"},
	"updateInterval":          {"Check interval", "確認間隔"},
	"hoursDetail":             {"Hours", "時間"},
	"codexAccounts":           {"Codex accounts", "Codexアカウント"},
	"codexAccount":            {"Codex account", "Codexアカウント"},
	"claudeAccounts":          {"Claude Code accounts", "Claude Codeアカウント"},
	"claudeAccount":           {"Claude Code account", "Claude Codeアカウント"},
	"profilePath":             {"Profile directory", "プロファイルディレクトリ"},
	"profilePathDetail":       {"“auto” uses the standard CODEX_HOME", "「auto」は標準のCODEX_HOMEを使用"},
	"claudeProfilePathDetail": {"“auto” uses CLAUDE_CONFIG_DIR or ~/.claude", "「auto」はCLAUDE_CONFIG_DIRまたは~/.claudeを使用"},
	"profileLabel":            {"Display name", "表示名"},
	"profileLabelDetail":      {"Optional name shown in menus and charts", "メニューとグラフに表示する任意の名前"},
	"profileShort":            {"Icon abbreviation", "アイコンの略称"},
	"profileShortDetail":      {"Optional, up to two characters", "任意、2文字まで"},
	"addAccount":              {"Add account", "アカウントを追加"},
	"removeAccount":           {"Remove this account", "このアカウントを削除"},
	"moveAccountUp":           {"↑ Move earlier", "↑ 前に移動"},
	"moveAccountDown":         {"↓ Move later", "↓ 後ろに移動"},
	"lightColors":             {"Light mode colors", "ライトモードの配色"},
	"darkColors":              {"Dark mode colors", "ダークモードの配色"},
	"colorGood":               {"Normal", "正常"},
	"colorWarn":               {"Warning", "警告"},
	"colorCritical":           {"Critical", "危険"},
	"colorUnknown":            {"Unknown", "不明"},
	"colorClaude":             {"Claude Code", "Claude Code"},
	"colorCodex":              {"Codex", "Codex"},
	"colorPeriod":             {"Period label", "期間ラベル"},
	"colorTextOnFill":         {"Text on fill", "塗りつぶし上の文字"},
	"colorThresholds":         {"Color thresholds", "配色のしきい値"},
	"warnBelow":               {"Warning below", "警告にする残量"},
	"criticalBelow":           {"Critical below", "危険にする残量"},
	"percentDetail":           {"Remaining percent (1–100)", "残量%（1〜100）"},
	"overview":                {"Overview", "概要"},
	"settingsNavigation":      {"Settings sections", "設定カテゴリ"},
	"generalSettings":         {"General", "一般"},
	"accountSettings":         {"Accounts", "アカウント"},
	"alertsAndData":           {"Alerts & data", "通知とデータ"},
	"appearanceSettings":      {"Appearance", "外観"},
	"saved":                   {"Saved", "保存しました"},
	"saveFailed":              {"Could not save", "保存できませんでした"},
}
...
func (p Printer) TranslateNote(note string) string {
	...
	replacements := []struct{ from, to string }{
		...
		{"no figures reported", "取得値なし"},
		{"no figures: usage cache ", "取得値なし: 使用量キャッシュ "},
		{"no figures: /usage ", "取得値なし: /usage "},
